...
// import modules to write history String to file
import java.io.File;
...
File historyFile = new File("history_file.txt");
...
void loop()
{
    ...
    int autoButton = digitalRead(3);
    boolean l = !left && right;
    boolean r = !right && left;
    boolean f = !left && !right;
    boolean s = left && right;
    boolean a = !autoButton;
    ...
    if(blueToothSerial.available()) {   // Print recvChar to blueToothSerial for slave to read
      blueToothSerial.print(recvChar);
    }
...
void automatedMaster() {
    while(1) {
        char sendChar;
        if(blueToothSerial.available()) {
            sendChar = blueToothSerial.read(); // reads sendChar from slave
            ...
            // if end of maze reached sound buzzer
            else {
              while (1) {
                tone(4, 1000, 1000); // 1000Hz for 1 sec, delay 1 sec
                delay(1000);
              }
            }
