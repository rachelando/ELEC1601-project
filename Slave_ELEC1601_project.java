...
void loop()
{
    char recvChar;
        ...
        if(blueToothSerial.available())   // Check if there's any data sent from the remote Bluetooth shield
        {
            ...
            if (recvChar == 'Z') {
              Serial.println("The robot has entered automated mode.");
              automated();
            }
...
void automated() {
    while(1) {
        ...
        if (endReached) {                // if robot reaches the end, send signal to master to save data (endReached NOT IMPLEMENTED)
          sendChar = 'E';
        }

        if(blueToothSerial.available()) {   // send automated char back to master
